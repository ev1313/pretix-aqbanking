...
//
// into output and output_length the string and it's size respectively will be
// written.
//
// input_str may be NULL, an empty json string is then returned.
static bool escape_json(const char *input_str, char **output,
  ...
  /* copy the string */
  for (input_pointer = input_str; *input_pointer != '\0';
  ...
  if (accs) {
    ...
    if (it) {
      ...
      while (a) {
        ...
        fprintf(stdout, "Account %d: %s (%s) %s (%s) [%s]\n", ++i,
                AB_Account_GetBankCode(a), AB_Account_GetBankName(a),
                AB_Account_GetAccountNumber(a), AB_Account_GetAccountName(a),
...
void list_transactions(AB_BANKING *ab, AB_ACCOUNT *a, int send,
                       const char *pretix_event, const char *pretix_url,
                       const char *pretix_token) {
  if (!a) {
  ...
  while (ai) {
    ...
    while (t) {
      ...
      if (v) {
        ...
        unsigned long trans_hash = hash(trans_remote_name) +
                                   hash(trans_purpose) + hash(trans_date) +
                                   hash(trans_currency);

        if (!send) {
          fprintf(stdout,
                  "transaction %lu - name: (%s) purpose: (%s) date: (%s) "
                  "value: (%.2f %s)\n",
                  trans_hash, trans_remote_name, trans_purpose, trans_date,
                  trans_value, trans_currency);
        } else {
          ...
          if (curl) {
            ...
            char token_str[1024];
            snprintf(token_str, sizeof(token_str), "Authorization: Token %s",
                     pretix_token);
            ...
            escape_json(trans_remote_name, &json_trans_remote_name,
                        &json_trans_remote_name_size);

            char *json_trans_purpose = NULL;
            size_t json_trans_purpose_size = 0;
            escape_json(trans_purpose, &json_trans_purpose,
                        &json_trans_purpose_size);

            snprintf(json_str, sizeof(json_str),
                     "{ \"event\": \"%s\", \"transactions\" : [ { \"payer\": "
                     "%s, \"reference\": %s, \"amount\": \"%.2f\", \"date\": "
                     "\"%s\" } ] }",
                     pretix_event, json_trans_remote_name, json_trans_purpose,
...
int main(int argc, char **argv) {
  ...
  if (argc > 0) {
    if (argc == 1 || strcmp(argv[1], "--help") == 0) {
      ...
      fprintf(stdout,
              "--list_transactions [Account Number (wildcard possible)] - "
              "lists transactions from account identified by account number\n");
      fprintf(stdout,
              "--send_transactions [Account Number (wildcard possible)] "
              "[pretix event name] [pretix api url] [pretix api token] - send "
              "transactions from account to pretix\n");
      return 0;
    }
  }
